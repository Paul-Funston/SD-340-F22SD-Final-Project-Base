...
namespace SD_340_W22SD_Final_Project_Group6.BLL
{
    public class TicketBL
    {
        ...
        public async Task<Ticket> GetTicketDetails(int? id)
        {
            ...
            ticket.TicketWatchers = _ticketWatcherRepo.GetAll().Where(x => x.TicketId == ticket.Id).ToList();
            ticket.Owner = _userRepo.Get(int.Parse(ticket.Owner.Id));
            ticket.Comments = _commentRepo.GetAll().Where(x => x.TicketId == ticket.Id).ToList();
